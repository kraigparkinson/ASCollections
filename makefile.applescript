...
script api
	...
	shell for "env LANG=en_US.UTF-8 headerdoc2html" given options:{"-q", "-o", dir, "ASCollections.applescript"}
	shell for "env LANG=en_US.UTF-8 gatherheaderdoc" given options:dir
end script

script BuildCollections
	property parent : Task(me)
	property name : "ddd"
...
script clobber
	...
	tell clean to exec:{}
	removeItems at glob({"ASCollections-*", "*.tar.gz", "*.html"}) with forcing
end script
...
script dist
	...
	set {n, v} to {name, version} of Â
		(run script POSIX file (joinPath(workingDirectory(), "src/ASCollections.applescript")))
	set dir to n & "-" & v
	makePath(dir)
	copyItems at {"build/ASCollections.scptd", "COPYING", "Documentation", Â
...
script installScriptLibraries
	...
	property description : "Install ASCollections in" & space & dir
...
script BuildTests
	...
	makeScriptBundle from "test/Test ASCollections.applescript" at "test" with overwriting
end script
...
script RunTests
	...
	set testSuite to load script POSIX file (joinPath(workingDirectory(), "test/Test ASCollections.scptd"))
	run testSuite
end script
...
script uninstall
	...
	property dir : POSIX path of Â
		((path to library folder from user domain) as text) & "Script Libraries"
	property description : "Remove ASCollections from" & space & dir
	
	set targetPath to joinPath(dir, "com.kraigparkinson/ASCollections.scptd")
...
script VersionTask
	...
	property description : "Print ASCollections's version and exit"
	property printSuccess : false
	
	set {n, v} to {name, version} of Â
		(run script POSIX file (joinPath(workingDirectory(), "ASCollections.applescript")))
	ohai(n & space & "v" & v)
end script
